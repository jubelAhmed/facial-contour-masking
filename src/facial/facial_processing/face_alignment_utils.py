...
def rotate_and_crop_face(image, landmarks, angle=None, return_crop_box=False):
    ...
    cropped_landmarks = rotated_landmarks - np.array(
        [x1, y1]
    )  # Shift landmarks to cropped coordinates

    x_start, y_start, x_end, y_end = get_padded_bbox(
        rotated_landmarks, rotated_image.shape
    )
