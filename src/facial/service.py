...
from src.facial.models import ProcessingJob, PerceptualHash, ProcessingMetrics
from src.facial.schemas import (
    LandmarkPoint, ProcessingRequest, ProcessingResponse, JobStatusResponse
)
from src.facial.exceptions import (
    FacialProcessingException, InvalidImageException, InsufficientLandmarksException,
    JobNotFoundException
)
from src.facial.constants import ProcessingStatus, OutputFormat, RegionType
from src.facial.repository import (
    IProcessingJobRepository, ProcessingJobRepository,
    IPerceptualHashRepository, PerceptualHashRepository
)
...
class FacialProcessingService:
    ...
    def __init__(
        ...
        self.processor = FacialSegmentationProcessor()
    
    # ========== JOB MANAGEMENT METHODS ==========
    
    async def create_processing_job(self, user_id: int, request: ProcessingRequest) -> ProcessingResponse:
    ...
    async def _process_job_async(self, job_id: str, request: ProcessingRequest) -> None:
        ...
        try:
            ...
            if cached_result:
                ...
                # Cache result
                await self.hash_repository.create_hash(
                    cache_key, 
                    json.dumps(result)
                )
    ...
    async def _process_image(self, request: ProcessingRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            style_config = StyleConfigFactory.create_style_config(request.style or "default")
