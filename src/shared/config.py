...
class AuthConfig(BaseSettings):
    ...
    secret_key: str = Field(
        "your-secret-key-change-in-production", description="JWT secret key"
    )
    access_token_expire_minutes: int = Field(
        60, description="Access token expiry in minutes"
    )
    refresh_token_expire_days: int = Field(
        30, description="Refresh token expiry in days"
    )
...
class RateLimitConfig(BaseSettings):
    ...
    redis_url: Optional[str] = Field(
        None, description="Redis URL for rate limiting storage"
    )
